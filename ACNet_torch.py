...
class ActorCritic(nn.Module):
    def __init__(self,action_dim,training=True):
        ...
        # 在pytorch中，lstm是先hx,后cx，这和tensorflow正好相反。
        h_init = torch.zeros(1,RNN_SIZE)
        c_init = torch.zeros(1,RNN_SIZE)
    ...
    def forward(self,inputs,goal_pos,hidden):
        ...
        output_lstm,(hx, cx) = self.lstm(h3, (hidden[0],hidden[1]))
        # hx, cx = self.lstm(h3[0], hidden)
        # for i in range(h3.size(0)-1):
        #     hx, cx = self.lstm(h3[i+1], (hx, cx))
        # 这里初始化隐藏状态
        # hx, cx = self.lstm(h3, self.init_hidden(1))
        rnn_out = output_lstm

        policy = F.softmax(self.policy_linear(rnn_out))
        ...
        policy_sig = F.sigmoid(self.policy_linear(rnn_out))
